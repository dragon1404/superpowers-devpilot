# `/dev-fix-pipeline` Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the undrafted `dev-fix-pipeline` skill with a spec-compliant version that finds the latest pipeline build, classifies the failure, applies a fix via systematic-debugging, verifies locally, and pushes only when verification passes.

**Architecture:** Three SKILL.md files are touched. The new `skills/dev-fix-pipeline/SKILL.md` is a complete rewrite of the draft. `skills/dev-workitem/SKILL.md` gains two new state fields in its template. `README.md` is updated with the new command.

**Tech Stack:** SKILL.md instruction files, Azure DevOps MCP tools, superpowers:systematic-debugging

---

### Task 1: Add pipeline fix fields to dev-workitem state template

**Files:**
- Modify: `skills/dev-workitem/SKILL.md` (state file template in Step 6)

- [ ] **Step 1: Locate the state template**

Open `skills/dev-workitem/SKILL.md`. Find the JSON block in Step 6 — it currently ends with:
```json
  "prUrl": null,
...
Replace that closing section so the full JSON template becomes:
```json
{
  "workItemId": 0,
...
Also update the prose line below the block to include the new fields:
> Replace `0`, `"feature/0-slug"`, `worktreePath`, org/project/repo, and timestamp with actual values. `pipelineFixCount` starts at `0`. `lastPipelineFixAt` starts as `null`. Use ISO 8601 format for `lastUpdated`.

- [ ] **Step 3: Verify**

Read back the modified section and confirm both `pipelineFixCount` and `lastPipelineFixAt` appear in the template and the prose explanation is updated.

- [ ] **Step 4: Commit**

```bash
git add skills/dev-workitem/SKILL.md
git commit -m "feat: add pipelineFixCount and lastPipelineFixAt to state file template"
```

---

### Task 2: Replace skills/dev-fix-pipeline/SKILL.md

**Files:**
- Modify: `skills/dev-fix-pipeline/SKILL.md` (full rewrite)

- [ ] **Step 1: Overwrite the file with the spec-compliant content**

Write the following content exactly to `skills/dev-fix-pipeline/SKILL.md`:
...
Read the written SKILL.md against the spec. Confirm:
- [ ] All 3 preconditions checked (state file exists, `prCreated: true`, branch matches)
- [ ] Build discovery: top 1, falls back to `pipelines_list_runs`
- [ ] Status check: handles failed / in-progress / passed explicitly
- [ ] Log retrieval drills to timeline records if needed
- [ ] Classification table: automatable vs not-automatable
- [ ] Not-automatable → ADO comment + stop
- [ ] Debugging invoked with full context
- [ ] Post-debug `git diff --name-only` check → ADO comment + stop if empty
- [ ] Verification: tests first, sandbox-skip fallback, build check
- [ ] Stop conditions for test failure and build failure each post to ADO
- [ ] Push stages only changed files
- [ ] State file updated: `pipelineFixCount` incremented, `lastPipelineFixAt` set
- [ ] ADO comment at fix-pushed includes verification label
- [ ] Developer report includes fix count

- [ ] **Step 3: Commit**

```bash
git add skills/dev-fix-pipeline/SKILL.md
git commit -m "feat: rewrite dev-fix-pipeline skill per approved design spec"
```
...
- [ ] **Step 1: Update the state file example**

Find the state file JSON example in the README (under `## State File`). Add the two new fields after `"prUrl": null`:
```json
  "pipelineFixCount": 0,
  "lastPipelineFixAt": null,
```

- [ ] **Step 2: Update the Status values table**

The table already has the correct statuses. No change needed here.

- [ ] **Step 3: Update the `/dev-fix-pipeline` command description**

Find the existing `/dev-fix-pipeline` section (already present from the draft). Replace its **What it does** list with the spec-accurate steps:
...
- [ ] **Step 4: Update the superpowers skills table**

Ensure the table entry reads:
```
...
```bash
git add README.md
git commit -m "docs: update README for spec-compliant dev-fix-pipeline"
```
